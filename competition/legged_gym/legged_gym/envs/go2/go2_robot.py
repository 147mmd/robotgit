...
class Go2Robot(LeggedRobot):
    ...
    def _init_buffers(self):
        ...
        # xyz,quat,lin_vel,ang_vel是返回的13维状态
    ...
    def _get_env_origins(self):
        ...
        if self.cfg.terrain.mesh_type in ["heightfield", "trimesh"]:
            ...
            self.env_origins = torch.zeros(self.num_envs, 3, device=self.device, requires_grad=False) #创建了一个大小为 (self.num_envs, 3) 的零张量。
            #self.num_envs 是张量的行数，即环境的数量。
            #3 是张量的列数。代表x，y，z的坐标
            #本质上env_origins是个二维张量
            self.env_origins[:,0:1] = torch_rand_float(0, 48, (self.num_envs,1), device=self.device) #设置每个环境的初始x坐标
            self.env_origins[:,1:2] = torch_rand_float(4, 8, (self.num_envs,1), device=self.device) #设置每个环境的初始z坐标
    ...
    def _get_heights(self, env_ids=None):
        ...
        #pos = x,y
        # 将脚的参数实际上的m，转换到所用坐标系的维度，要除scale,self.height_samples[x/0.25,y/0.25]
    ...
    #    #self.foot_handles = self.gym.find_asset_rigid_body_index(self.robot_asset, "hip_names") #从urdf中导入各个位置
    #     #self.foot_pos
    #     # heights
